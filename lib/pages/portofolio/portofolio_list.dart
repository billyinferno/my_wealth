  ...
  void initState() {
    ...
    Future.microtask(() async {
      ...
      await _portofolioAPI.getPortofolioSummary(_args.type).then((resp) {
        ...
        int index = 0;
        for (PortofolioSummaryModel porto in resp) {
          _barChartData.add(BarChartData(title: porto.portofolioCompanyType, value: porto.portofolioTotalValue, total: _portofolioTotalValue, color: Globals.colorList[index]));
          index = index + 1;
        }
      }).whenComplete(() {
        // remove the loader
            ...
            Column(
              ...
              children: List<Widget>.generate(_portofolioList.length, ((index) {
                return ProductListItem(
                  bgColor: Globals.colorList[index],
                  title: _portofolioList[index].portofolioCompanyType,
