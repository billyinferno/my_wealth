...
class _WatchlistSummaryCalendarPageState extends State<WatchlistSummaryCalendarPage> {
  ...
  Future<void> _getPerformanceDataMonthYear({
    ...
    if (type == 'all') {
      ...
      // check end date, whether end date is more than today+1 or not?
      // this is to avoid we put the data until end of month, where we don't
      // reach that date yet.
      DateTime todayDate = DateTime.now().add(const Duration(days: 1));
