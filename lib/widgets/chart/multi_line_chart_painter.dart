...
class MultiLineChartPainter extends CustomPainter {
  ...
  const MultiLineChartPainter({required this.legend, required this.data});
  ...
  void _drawLine(Canvas canvas, Size size, Offset center) {
    ...
    //   ..strokeWidth = 1.0
    //   ..style = PaintingStyle.stroke;
    // Paint watchlistPaintBuy = Paint()..color = accentColor;
    // Paint watchlistPaintSell = Paint()..color = extendedLight;
    // Paint avgPricePaint = Paint()..color = Colors.orange.withOpacity(0.7);
    // Paint ma5PricePaint = Paint()..color = Colors.green.withOpacity(0.7);
    // Paint ma8PricePaint = Paint()..color = Colors.pink.withOpacity(0.7);
    // Paint ma13PricePaint = Paint()..color = Colors.blue.withOpacity(0.7);
    ...
    //         if (watchlist![value.date.toLocal()] == 1 ||
    //             watchlist![value.date.toLocal()] == 3) {
  ...
  void _drawDashedLine(
      Canvas canvas, Rect graph, double width, double offset, Paint paint) {
    int j = 0;
    for (double i = graph.left; i <= graph.right; i = (i + width)) {
      if (j % 2 == 0) {
        canvas.drawLine(Offset(i, graph.bottom - offset),
            Offset((i + width), graph.bottom - offset), paint);
      }
